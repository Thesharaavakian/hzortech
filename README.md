...
| `/services/` | 0.9 | Service index with links to all subpages |
| `/services/{slug}/` | 0.85 | Dedicated service landing pages (×10) |
| `/projects/` | 0.9 | Portfolio — named client engagements |
| `/about/` | 0.8 | Company & founder information |
| `/contact/` | 0.7 | Contact + FAQ schema |
| `/blog/` | 0.6 | Technical insights |

Sitemap at `/sitemap.xml` — generated by Django's sitemaps framework.

### Service Landing Pages

10 dedicated pages at `/services/{slug}/`, each with unique content, FAQ schema, BreadcrumbList, and internal links:

| Slug | Service | Primary Keywords |
|---|---|---|
| `software-development` | Custom Software | software developers Armenia, Django Yerevan |
| `devops` | DevOps & CI/CD | DevOps Armenia, CI/CD Armenia, Kubernetes |
| `crm-automation` | CRM & Automation | CRM automation Armenia, HubSpot Armenia |
| `smart-home-automation` | Smart Home & IoT | smart home Armenia, IoT Armenia |
| `cloud` | Cloud Infrastructure | cloud engineering Armenia, AWS Armenia |
| `python` | Python Development | Python developers Armenia, Django Armenia |
| `api-integration` | API Integration | API integration Armenia, system integration |
| `security` | Cybersecurity | cybersecurity Armenia, SIEM Armenia |
| `cloud-migration` | Cloud Migration | cloud migration Armenia, AWS migration |
| `hpc-linux` | HPC & Linux | HPC Armenia, Linux engineer Armenia |

All content lives in `business_page/services_data.py` (programmatic SEO — one template, data-driven pages).

### Structured Data

- **All pages:** `Organization` + `LocalBusiness` + `ProfessionalService` + `WebSite` (with `SearchAction`)
- **Home:** `WebPage` + `ItemList` (10 services with correct URLs)
- **Service detail:** `Service` + `WebPage` + `FAQPage` + `BreadcrumbList`
- **About:** `AboutPage` + `Person` (founder + `alumniOf` + `knowsAbout`)
- **Projects:** `CollectionPage` + `ItemList` (6 named engagements)
- **Contact:** `ContactPage` + `FAQPage` (5 entries targeting "hire engineers Armenia")

### Local SEO Signals

- `geo.region: AM-ER`, `geo.placename: Yerevan, Armenia`, `ICBM: 40.179186, 44.499103`
- Organization schema: `addressLocality: Yerevan, addressCountry: AM`
- `hreflang`: en / hy (Armenian) / ru / x-default
- Natural Armenia/Yerevan in every page's meta description and hero content
